  ...
  try {
    ...
    for (const row of dataRows) {
      ...
      try {
        ...
        const {rows: exist} = await client.query(`SELECT id FROM productions_jour WHERE date_production=$1`, [date]);
        let pjId;
        if (exist[0]) {
...
router.post('/stocks', auth, role(DG), upload.single('fichier'), async (req, res) => {
  try {
    ...
    const sheets = wb.SheetNames.filter(n=>
      n.toUpperCase().includes('CLASSE') || n.toUpperCase().includes('CONSOMMABLE') ||
      n.toUpperCase().includes('EPI') || n.toUpperCase().includes('PIECE') || n.toUpperCase().includes('PRODUIT')
    );
    ...
    for (const sheetName of sheets) {
      ...
      for (const row of dataRows) {
        ...
        const prix=num(row[4]); const debut=num(row[5]);
        const sorties=num(row[6]); const entrees=num(row[7]);
        if (!code || !libelle) { skipped++; continue; }
        if (debut===0 && sorties===0 && entrees===0) { skipped++; continue; }
        try {
          const {rows:art} = await pool.query(
            `INSERT INTO stocks_articles (code,libelle,unite,classe,prix_unitaire_ht,actif) VALUES ($1,$2,$3,$4,$5,true)
             ON CONFLICT (code) DO UPDATE SET libelle=$2,unite=$3,classe=$4,
             prix_unitaire_ht=CASE WHEN $5>0 THEN $5 ELSE stocks_articles.prix_unitaire_ht END,actif=true RETURNING id`,
            [code, libelle, unite, classe, prix]
          );
          const artId = art[0].id;
          if (debut>0) await pool.query(`INSERT INTO stocks_mouvements (article_id,type_mouvement,quantite,date_mouvement,motif,saisi_par_id) VALUES ($1,'entree',$2,$3,'Report solde mois précédent',$4)`,[artId,debut,dateMois,req.user.id]);
          if (sorties>0) await pool.query(`INSERT INTO stocks_mouvements (article_id,type_mouvement,quantite,date_mouvement,motif,saisi_par_id) VALUES ($1,'sortie',$2,$3,'Vente / Livraison client',$4)`,[artId,sorties,dateMois,req.user.id]);
          if (entrees>0) await pool.query(`INSERT INTO stocks_mouvements (article_id,type_mouvement,quantite,date_mouvement,motif,saisi_par_id) VALUES ($1,'entree',$2,$3,'Production validée',$4)`,[artId,entrees,dateMois,req.user.id]);
          inserted++;
        } catch(e) { erreurs.push(`${code}: ${e.message}`); console.error('[IMPORT STK ROW]', code, e.message); }
      }
    }
...
// DELETE /import/historique — effacer tout
router.delete('/historique', auth, role(DG), async (req, res) => {
  try {
    await pool.query('DELETE FROM import_historique');
    res.json({message:'Historique effacé ✓'});
  } catch(e) { res.status(500).json({message:e.message}); }
});

// DELETE /import/historique/:id — effacer une ligne
router.delete('/historique/:id', auth, role(DG), async (req, res) => {
  try {
    await pool.query('DELETE FROM import_historique WHERE id=$1',[req.params.id]);
