...
// DELETE /api/rapports/historique — effacer tout
router.delete('/historique', auth, role(DG), async (req, res) => {
  try {
...
router.post('/generer', auth, async (req, res) => {
  try {
    ...
    try {
      const { rows: dg } = await pool.query(`SELECT nom_complet FROM utilisateurs WHERE id=$1`, [req.user.id]);
    ...
    if (['production', 'rebuts'].includes(type_rapport)) {
      ...
      try {
        ...
        pjRows = r.rows;
      } catch(e) { console.error('[RAPPORT] productions_jour:', e.message); }
        ...
        try {
          const lr = await pool.query(
            `SELECT fp.code, lp.cartons_produits FROM lignes_production lp
             JOIN formats_produits fp ON fp.id=lp.format_id WHERE lp.production_id=$1`, [pj.id]
          );
          lr.rows.forEach(l => { const k=mapCode(l.code); if(k) prods[k]=parseInt(l.cartons_produits)||0; });
        } catch(e) { console.error('[RAPPORT] lignes_production:', e.message); }
        try {
          const rr = await pool.query(
            `SELECT pref32,pref17,bouchons,ctn_c12,ctn_c24,hilio_rebut,
          ...
          if (rr.rows[0]) {
            const rb = rr.rows[0];
            rebuts = {
              pref32:parseInt(rb.pref32)||0, pref17:parseInt(rb.pref17)||0,
              bouchons:parseInt(rb.bouchons)||0, ctn_c12:parseInt(rb.ctn_c12)||0,
              ctn_c24:parseInt(rb.ctn_c24)||0, hilio:parseInt(rb.hilio_rebut)||0,
              etiq_c12:parseInt(rb.etiq_c12)||0, etiq_c24:parseInt(rb.etiq_c24)||0,
            };
          }
        } catch(e) { console.error('[RAPPORT] rebuts:', e.message); }
        saisies.push({ date_production:pj.date_production, statut:pj.statut,
          jours_ouvres:pj.jours_ouvres, saisi_par_nom:'Opérateur',
          c12:prods.c12, c24:prods.c24, f615:prods.f615, f605:prods.f605, f61:prods.f61, hilio:prods.hilio, rebuts });
      }
      donnees.saisies = saisies;
      const valides = saisies.filter(s=>s.statut==='valide');
      donnees.totaux = {
        c12:valides.reduce((a,s)=>a+s.c12,0), c24:valides.reduce((a,s)=>a+s.c24,0),
        f615:valides.reduce((a,s)=>a+s.f615,0), f605:valides.reduce((a,s)=>a+s.f605,0),
        f61:valides.reduce((a,s)=>a+s.f61,0), hilio:valides.reduce((a,s)=>a+s.hilio,0),
      };
      ...
      valides.forEach(s=>{
        ...
        const rb=s.rebuts;
        rebCum.PREF_32G=(rebCum.PREF_32G||0)+rb.pref32;
        rebCum.PREF_17G=(rebCum.PREF_17G||0)+rb.pref17;
        ...
        rebCum.ETI_C12=(rebCum.ETI_C12||0)+rb.etiq_c12;
        rebCum.ETI_C24=(rebCum.ETI_C24||0)+rb.etiq_c24;
      });
      donnees.consommations_cumulees=consCum; donnees.rebuts_cumules=rebCum;
      donnees.rebuts=valides.flatMap(s=>[
        ...
        {date:s.date_production,intrant:'Étiq C12 (1,5L)',quantite:s.rebuts.etiq_c12,prix:9,valeur:s.rebuts.etiq_c12*9},
        {date:s.date_production,intrant:'Étiq C24 (0,5L)',quantite:s.rebuts.etiq_c24,prix:6,valeur:s.rebuts.etiq_c24*6},
    ...
    if (type_rapport==='atp') {
      try {
        ...
        if (donnees.atp.id) {
          const {rows:objRows}=await pool.query(`SELECT code_produit,quantite FROM atp_objectifs WHERE atp_id=$1`,[donnees.atp.id]);
        ...
        } else { donnees.objectifs={C12:0,C24:0,F615:0,F605:0,F61:0,HILIO:0}; }
      ...
      } catch(e) { console.error('[RAPPORT ATP]',e.message); donnees.atp={}; donnees.objectifs={}; donnees.realisations={}; donnees.charges={}; donnees.totalCI=0; }
    }

    if (type_rapport==='stocks') {
      try {
        const {rows:articles}=await pool.query(`
          SELECT sa.*,COALESCE(SUM(CASE WHEN sm.type_mouvement='entree' THEN sm.quantite WHEN sm.type_mouvement='sortie' THEN -sm.quantite ELSE 0 END),0) AS stock_actuel,
          COALESCE(SUM(CASE WHEN sm.type_mouvement='entree' THEN sm.quantite WHEN sm.type_mouvement='sortie' THEN -sm.quantite ELSE 0 END),0)*sa.prix_unitaire_ht AS valeur_stock_ht
          FROM stocks_articles sa LEFT JOIN stocks_mouvements sm ON sm.article_id=sa.id WHERE sa.actif=true GROUP BY sa.id ORDER BY sa.classe,sa.libelle`);
        donnees.articles=articles;
        const {rows:mvts}=await pool.query(`
          SELECT sm.*,sa.libelle AS article_libelle,sa.classe,sa.unite,sm.quantite*sa.prix_unitaire_ht AS valeur_ht
      ...
      } catch(e) { donnees.articles=[]; donnees.mouvements=[]; }
    }

    if (type_rapport==='tresorerie') {
      try {
        const {rows:comptes}=await pool.query(`
          SELECT c.*,COALESCE(SUM(CASE WHEN m.sens='credit' THEN m.montant_fcfa WHEN m.sens='debit' THEN -m.montant_fcfa ELSE 0 END),0) AS solde_fcfa
          FROM comptes_tresorerie c LEFT JOIN tresorerie_mouvements m ON m.compte_id=c.id WHERE c.actif=true GROUP BY c.id ORDER BY c.libelle`);
        donnees.comptes=comptes;
        const {rows:mvts}=await pool.query(`
          SELECT m.*,c.libelle AS compte_libelle FROM tresorerie_mouvements m
        ...
        donnees.mouvements=mvts;
      } catch(e) { donnees.comptes=[]; donnees.mouvements=[]; }
    }

    const isExcel=format==='Excel'||format==='excel';
    const buffer=isExcel?await genererExcel(type_rapport,donnees,moisRap):await genererPDF(type_rapport,donnees,moisRap);
    ...
    pool.query(`INSERT INTO rapports (type_rapport,titre,periode_debut,genere_par_id,statut_envoi) VALUES ($1,$2,$3,$4,'genere')`,
      [type_rapport,`Rapport ${type_rapport} ${moisRap}`,moisRap,req.user.id]).catch(()=>{});
