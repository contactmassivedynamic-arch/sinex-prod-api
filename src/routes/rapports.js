  ...
  try {
    ...
    if (['production', 'rebuts'].includes(type_rapport)) {
      ...
      try {
        ...
        pjRows = r.rows;
      } catch(e) { console.error('[RAPPORT] SELECT productions_jour:', e.message); }
      ...
      for (const pj of pjRows) {
        ...
        saisies.push({
          ...
          c12:   prods.c12, c24:   prods.c24,
          f615:  prods.f615, f605: prods.f605,
          f61:   prods.f61, hilio: prods.hilio,
          rebuts,
        });
      }
      ...
      valides.forEach(s => {
        ...
        } catch(e) { console.error('[RAPPORT] calcConsommations:', e.message); }
      ...
      // Liste rebuts pour rapport — étiquettes séparées
    ...
    if (type_rapport === 'atp') {
      try {
        ...
        if (donnees.atp.id) {
          const { rows: objRows } = await pool.query(`SELECT code_produit, quantite FROM atp_objectifs WHERE atp_id=$1`, [donnees.atp.id]).catch(()=>({rows:[]}));
    ...
    if (type_rapport === 'tresorerie') {
      try {
        ...
        const { rows: mvts } = await pool.query(`
          ...
          LEFT JOIN tresorerie_mouvements m2 ON m2.compte_id=m.compte_id AND (m2.date_mouvement<m.date_mouvement OR (m2.date_mouvement=m.date_mouvement AND m2.id<=m.id))
