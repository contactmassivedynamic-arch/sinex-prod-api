...
router.get('/soldes', auth, async (req, res) => {
  try {
    const {rows} = await pool.query(`
      SELECT c.id, c.code, c.libelle, c.type_compte,
        COALESCE(SUM(
      ...
      WHERE c.actif=true
      GROUP BY c.id, c.code, c.libelle, c.type_compte
      ORDER BY c.type_compte DESC, c.libelle
    `);
    res.json(rows);
  } catch(err) {
    console.error('[TRES SOLDES]', err.message);
  ...
  try {
    ...
    let q = `
      SELECT m.id, m.sens, m.montant_fcfa, m.date_mouvement,
        m.description, m.nature_operation, m.piece_justificative, m.created_at,
    ...
    console.error('[TRES MVTS]', err.message);
    res.status(500).json({message: err.message});
  }
});
...
router.post('/mouvements', auth, async (req, res) => {
  try {
    const {compte_id, sens, montant_fcfa, date_mouvement, description, nature_operation, piece_justificative} = req.body;
    await pool.query(
      ...
      [compte_id, sens, montant_fcfa, date_mouvement||new Date(),
       description||'', nature_operation||'', piece_justificative||'', req.user.id]
    );
    res.status(201).json({message:'Mouvement enregistré ✓'});
  } catch(err) {
    console.error('[TRES POST]', err.message);
    res.status(500).json({message: err.message});
  }
});

// PUT /api/tresorerie/mouvements/:id
router.put('/mouvements/:id', auth, role(DG), async (req, res) => {
  try {
    const {sens, montant_fcfa, date_mouvement, description, nature_operation} = req.body;
    await pool.query(
      `UPDATE tresorerie_mouvements
       SET sens=$1, montant_fcfa=$2, date_mouvement=$3, description=$4, nature_operation=$5
       WHERE id=$6`,
      [sens, montant_fcfa, date_mouvement, description, nature_operation, req.params.id]
    );
...
// DELETE /api/tresorerie/mouvements/:id
...
router.get('/flux', auth, async (req, res) => {
  try {
    ...
    console.error('[TRES FLUX]', err.message);
    res.status(500).json({message: err.message});
  }
});
