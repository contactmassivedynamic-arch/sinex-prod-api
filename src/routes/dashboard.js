  ...
  try {
    ...
    const {rows:jr}=await pool.query(
      `SELECT COALESCE(SUM(jours_ouvres),0) AS jours FROM productions_jour
       WHERE TO_CHAR(date_production,'YYYY-MM')=$1 AND statut='valide'`,[mois]
    );
    ...
    // Évolution — TOUTE l'année en cours (janvier à décembre)
    const {rows:evoRows} = await pool.query(
      `SELECT TO_CHAR(pj.date_production,'YYYY-MM') AS mois, fp.code, SUM(lp.cartons_produits) AS total
       ...
       WHERE pj.statut='valide'
         AND TO_CHAR(pj.date_production,'YYYY') = $1
       GROUP BY mois, fp.code ORDER BY mois`,
      [annee]
    );
    const evoMap={};
    evoRows.forEach(r=>{
      if(!evoMap[r.mois]) evoMap[r.mois]={mois:r.mois,c12:0,c24:0,hilio:0,f615:0,f605:0,f61:0};
      const key = r.code.toLowerCase();
      if(evoMap[r.mois][key]!==undefined) evoMap[r.mois][key]=parseInt(r.total);
    });
