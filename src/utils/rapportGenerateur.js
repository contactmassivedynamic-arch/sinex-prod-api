...
const fmt  = n => { const v=Math.round(parseFloat(n)||0); return v.toLocaleString('fr-FR').replace(/\u202f/g,' ').replace(/\u00a0/g,' '); };
...
const C = {
  fond:'#0F172A', header:'#1E293B', section:'#334155', accent:'#475569',
  texte:'#1E293B', clair:'#F8FAFC', blanc:'#FFFFFF', bordure:'#CBD5E1',
  vert:'#166534', rouge:'#991B1B', ambre:'#92400E', cyan:'#0891B2',
};

const TITRES = {
  production:'RAPPORT DE PRODUCTION MENSUEL', atp:'RAPPORT FINANCIER ATP',
  stocks:'ÉTAT DES STOCKS', tresorerie:'RAPPORT DE TRÉSORERIE',
  rebuts:'RAPPORT DES REBUTS', tendances:'ANALYSE DES TENDANCES',
};
...
function xlHeader(ws, titre, ncols, mois) {
  ...
  ct.value = titre + '  —  ' + mois.replace('-',' / ') + '  —  ' + new Date().toLocaleDateString('fr-FR');
...
async function xlProduction(wb, d, mois) {
  ...
  xlHeader(ws,'RAPPORT DE PRODUCTION MENSUEL',10,mois);
  let sec=1;
...
async function xlATP(wb, d, mois) {
  ...
  xlHeader(ws,'RAPPORT FINANCIER ATP',4,mois);
  let sec=1;
  ...
  ['C24','C12','F605','F615','F61','HILIO'].forEach(c=>{
    const q=obj[c]||0; xlLigne(ws,[c,fmt(q),fmt(PRIX_PF[c]||0),fmt(q*(PRIX_PF[c]||0))]);
  });
  ...
  const mbhtpXl=cahtp-cdhtp,bmfPxl=mbhtpXl*(15/35),fsPxl=mbhtpXl*(10/35);
  xlLigne(ws,['BMF',fmt(bmfPxl),fmtP(tmbhtp*(15/35)),'']);
  xlLigne(ws,['Frais de Siège',fmt(fsPxl),fmtP(tmbhtp*(10/35)),'']);
  xlLigne(ws,['Amortissement',fmt(fsPxl),fmtP(tmbhtp*(10/35)),'']);
  xlLigne(ws,['TOTAL',fmt(bmfPxl+fsPxl+fsPxl),fmtP(tmbhtp*(35/35)),''],{bold:true,bg:'#0F172A',color:'#F8FAFC'});
  ...
  const mbhtrXl=cahtr-cdhtr,bmfRxl=mbhtrXl*(15/35),fsRxl=mbhtrXl*(10/35);
  xlLigne(ws,['BMF',fmt(bmfRxl),fmtP(tmbhtr*(15/35)),'']);
  xlLigne(ws,['Frais de Siège',fmt(fsRxl),fmtP(tmbhtr*(10/35)),'']);
  xlLigne(ws,['Amortissement',fmt(fsRxl),fmtP(tmbhtr*(10/35)),'']);
  xlLigne(ws,['TOTAL',fmt(bmfRxl+fsRxl+fsRxl),fmtP(tmbhtr*(35/35)),''],{bold:true,bg:'#0F172A',color:'#F8FAFC'});
...
async function xlStocks(wb, d, mois) {
  ...
  xlHeader(ws1,'ÉTAT DES STOCKS',9,mois);
  let sec=1;
  [1,2,3].forEach(cl=>{
  ...
  xlHeader(ws2,'MOUVEMENTS DE STOCK',7,mois);
...
async function xlTresorerie(wb, d, mois) {
  ...
  xlHeader(ws,'RAPPORT DE TRÉSORERIE',6,mois);
...
async function xlRebuts(wb, d, mois) {
  ...
  xlHeader(ws,'RAPPORT DES REBUTS',5,mois);
  ...
  if(rebuts.length===0){
    ...
    xlLigne(ws,['TOTAL','',fmt(rebuts.reduce((s,r)=>s+(r.quantite||0),0)),'',fmt(rebuts.reduce((s,r)=>s+(r.valeur||0),0))],{bold:true,bg:'#0F172A',color:'#F8FAFC'});
  }
...
async function xlTendances(wb, d, mois) {
  ...
  xlHeader(ws,'ANALYSE DES TENDANCES',4,mois);
  let sec=1;
  ...
  xlSection(ws,sec++,'Historique CA et MB',4);
  xlEntete(ws,['Mois','CA HT (FCFA)','MB HT (FCFA)','TMB']);
  hist.forEach(m=>{ xlLigne(ws,[m.mois,fmt(m.ca_ht||0),fmt(m.mb_ht||0),fmtP(m.tmb||0)]); });
  xlLigne(ws,['MOYENNE',fmt(caMoyen),fmt(mbTotal/nbMois),fmtP(tmbMoyen)],{bold:true,bg:'#0F172A',color:'#F8FAFC'});
  xlSection(ws,sec++,'Capacité de remboursement',4);
  ...
  xlLigne(ws,['Excédent Brut Exploitation',fmt(excedent),'',''],{bold:true});
  xlLigne(ws,['Capacité de remboursement mensuelle',fmt(capaciteRemb),'',''],{bold:true,bg:'#0F172A',color:'#F8FAFC'});
...
function drawWatermark(doc) {
  try {
    ...
    const x = (W - logoSize) / 2, y = (H - logoSize) / 2;
    doc.save();
    doc.opacity(0.13);
...
function pdfEntete(doc, type, mois, dgNom) {
  ...
  try { if (fs.existsSync(LOGO_PATH)) doc.image(LOGO_PATH, 15, 10, {width:80, height:80}); } catch(e) {}
  const tx = 105;
...
function pdfSignature(doc, dgNom) {
  ...
  try {
    if (fs.existsSync(SIG_PATH)) {
      ...
      doc.roundedRect(doc.page.width-205, sy, 160, 65, 5).lineWidth(1.5).stroke('#0F172A');
      doc.fillColor('#0F172A').fontSize(7.5).font('Helvetica-Bold')
         .text('SINEX SA — CACHET OFFICIEL', doc.page.width-203, sy+8, {width:156, align:'center'});
      doc.fillColor('#334155').fontSize(8.5).font('Helvetica-Bold')
         .text(dgNom||'Boumzina Raïna', doc.page.width-203, sy+22, {width:156, align:'center'});
      doc.fillColor('#0891B2').fontSize(7.5).font('Helvetica')
         .text('Directeur Général', doc.page.width-203, sy+35, {width:156, align:'center'});
      doc.fillColor('#64748B').fontSize(7).font('Helvetica')
         .text('Défalé, Togo — '+new Date().toLocaleDateString('fr-FR'), doc.page.width-203, sy+48, {width:156, align:'center'});
    }
  } catch(e) {}
  doc.moveDown(4);
...
function pdfProduction(doc, d) {
  resetSec();
  const saisies=d.saisies||[], valides=saisies.filter(s=>s.statut==='valide');
  ...
  pdfTableau(doc,
    ...
    saisies.map(s=>[
      ...
      s.jours_ouvres||1,
      {text:s.statut==='valide'?'Validé':'Attente',color:s.statut==='valide'?C.vert:C.ambre,bold:s.statut==='valide'},
    ]),
    [55,38,38,42,42,38,38,28,55]
  );
  pdfLigneTotale(doc,[
    `TOTAL — ${valides.length} validée(s) / ${saisies.length}`,
    fmt(saisies.reduce((a,s)=>a+(s.c12||0),0)),fmt(saisies.reduce((a,s)=>a+(s.c24||0),0)),
    fmt(saisies.reduce((a,s)=>a+(s.f615||0),0)),fmt(saisies.reduce((a,s)=>a+(s.f605||0),0)),
    fmt(saisies.reduce((a,s)=>a+(s.f61||0),0)),fmt(saisies.reduce((a,s)=>a+(s.hilio||0),0)),
    fmt(saisies.reduce((a,s)=>a+(parseFloat(s.jours_ouvres)||0),0)),'',
  ],[55,38,38,42,42,38,38,28,55]);
  ...
  pdfLigneTotale(doc,['TOTAL GÉNÉRAL',fmt(qtyTotal),fmt(caTotal),fmt(cdTotal),fmt(caTotal-cdTotal),caTotal>0?fmtP((caTotal-cdTotal)/caTotal):'—'],[55,45,95,95,95,55]);
...
function pdfATP(doc, d) {
  ...
  // Jauge avancement par format
  pdfSection(doc,'Avancement par format');
  ['C24','C12','F615','F605','F61','HILIO'].forEach(code=>{
    const q=real[code]||0, ob2=obj[code]||q*1.2||1;
    if(q>0||ob2>0) pdfJauge(doc,code,q,Math.max(ob2,q));
  });
}

function pdfJauge(doc,label,valeur,maximum) {
  if(doc.y>doc.page.height-40){doc.addPage();drawWatermark(doc);doc.moveDown(0.5);}
  const y=doc.y, x=49, W=doc.page.width-98, barH=10;
  const pct=maximum>0?Math.min(valeur/maximum,1):0;
  const col=pct>=0.9?C.vert:pct>=0.7?C.ambre:C.rouge;
  doc.fillColor('#E2E8F0').rect(x,y,W,barH).fill();
  doc.fillColor(col).rect(x,y,W*pct,barH).fill();
  doc.fillColor('#1E293B').fontSize(8).font('Helvetica').text(label,x,y+1,{width:40,lineBreak:false});
  doc.fillColor(col).fontSize(8).font('Helvetica-Bold').text(`${fmt(valeur)} / ${fmt(maximum)} (${(pct*100).toFixed(0)}%)`,x+W/4,y+1,{width:W*3/4,lineBreak:false,align:'right'});
  doc.moveDown(0.9);
}
...
function pdfStocks(doc, d) {
  ...
  [1,2,3].forEach(cl=>{
    ...
    if(!arts.length) return;
    const t={1:'Classe 1 — Consommables de production',2:'Classe 2 — EPI et Pièces de rechange',3:'Classe 3 — Produits finis'};
    const valeurTotCl=arts.reduce((a,x)=>a+parseFloat(x.valeur_stock_ht||0),0);
    const stockTotCl=arts.reduce((a,x)=>a+parseFloat(x.stock_actuel||0),0);
    const alertesCl=arts.filter(a=>{const s=parseFloat(a.stock_actuel||0);return s<=0||(a.seuil_alerte&&s<=a.seuil_alerte);}).length;
    pdfSection(doc,t[cl]);
    pdfTableau(doc,['Article','Code','Unité','Stock actuel','Valeur HT (FCFA)','Statut'],
      arts.map(a=>{
        ...
        return [a.libelle,a.code,a.unite,fmt(stock),fmt(a.valeur_stock_ht||0),{text:st,color:st==='Rupture'?C.rouge:st==='Faible'?C.ambre:C.vert,bold:true}];
      }),
      [135,62,42,52,85,58]
    );
    pdfLigneTotale(doc,[`TOTAL ${arts.length} articles — ${alertesCl} alerte(s)`,'','',fmt(stockTotCl),fmt(valeurTotCl),''],[135,62,42,52,85,58]);
  });
}
  ...
  pdfTableau(doc,['Date','Compte','Libellé','Entrée (FCFA)','Sortie (FCFA)','Solde'],
    (d.mouvements||[]).map(m=>{
      ...
      return [fmtD(m.date_mouvement),m.compte_libelle||'—',m.description||'—',
        isC?fmt(m.montant_fcfa||0):'—',!isC?fmt(m.montant_fcfa||0):'—',fmt(m.solde_apres||0)];
    }),
    [58,82,98,68,68,66]
  );
...
function pdfRebuts(doc, d) {
  ...
  if(rebuts.length===0){
    ...
    const totQty=rebuts.reduce((s,r)=>s+(r.quantite||0),0);
    pdfLigneTotale(doc,[`TOTAL — ${rebuts.length} ligne(s)`,fmt(totQty),'','',fmt(totVal)],[65,125,60,70,90]);
    doc.moveDown(0.5);
    doc.fillColor('#1E293B').fontSize(10).font('Helvetica-Bold')
       .text(`Valeur totale des rebuts : ${fmt(totVal)} FCFA`, 45, doc.y, {align:'right',width:doc.page.width-90});
  }
}
...
function pdfTendances(doc, d) {
  ...
  const cdTotal=hist.reduce((s,m)=>s+parseFloat(m.cd_ht||0),0);
  const mbTotal=hist.reduce((s,m)=>s+parseFloat(m.mb_ht||0),0);
  const caMoyen=caTotal/nbMois, mbMoyen=mbTotal/nbMois;
  const tmbMoyen=caTotal>0?mbTotal/caTotal:0;
  const HEURES_MOIS=416;
  ...
  pdfTableau(doc,['Parametre','Valeur','Detail'],
    [
      ["Heures productives / shift","8 h","Temps de production net"],
      ["Shifts / jour","2 shifts","Configuration actuelle"],
      ["Jours ouvres / mois","26 j","Base de calcul mensuelle"],
      ...
      ["Taux utilisation usine",d.jours_moyens>0?((d.jours_moyens/26)*100).toFixed(1)+" %":"--",d.jours_moyens>0&&(d.jours_moyens/26)>=0.85?"Bon niveau":"Marge d'optimisation possible"],
    ],
    [155,90,200]
  );

  pdfSection(doc,"Rendement par format de production");
  pdfTableau(doc,['Format','Rend. theo. (ctn/h)','Vol. moyen/mois','Cap. max/mois','Taux utilisation','Ecart potentiel'],
    Object.entries(REND_THEO).map(([code,rth])=>{
      const volReel=volMoyens[code]||0, capMax=CAP_MAX[code];
      const txUtil=capMax>0?volReel/capMax:0, ecart=capMax-volReel;
      return [
        code, rth+" ctn/h", fmt(volReel)+" ctn", fmt(capMax)+" ctn",
        {text:fmtP(txUtil),color:txUtil>=0.75?C.vert:txUtil>=0.50?C.ambre:C.rouge,bold:txUtil>=0.75},
        "+"+fmt(ecart)+" ctn",
      ];
    }),
    [55,80,75,75,75,80]
  ...
  const scenarios=[["A 70%",0.70],["A 80%",0.80],["A 90%",0.90],["A 100% (max)",1.00]];
  pdfTableau(doc,['Scenario','Vol. total (ctn)','CA HT estime (FCFA)','Gain vs actuel (FCFA)','MB estimee (FCFA)'],
    scenarios.map(([label,tx])=>{
      ...
      return [label,fmt(Math.round(volSim))+" ctn",fmt(Math.round(caSim)),{text:"+"+fmt(Math.round(caSim-caMoyen)),color:C.vert},fmt(Math.round(mbSim))];
    }),
    [115,75,95,95,80]
  );
  ...
  if(stocks_mp.length>0){
    ...
    pdfTableau(doc,['Matiere premiere','Stock actuel','Besoin mensuel','Couverture','Statut','Fournisseur'],
      stocks_mp.map(mp=>{
        const couv=mp.besoin_mensuel>0?mp.stock/mp.besoin_mensuel:0;
        return [
          mp.libelle, fmt(mp.stock)+" "+mp.unite, fmt(mp.besoin_mensuel)+" "+mp.unite,
          couv.toFixed(1)+" mois",
          {text:couv>=2?"OK":couv>=1?"Surveiller":"CRITIQUE",color:couv>=2?C.vert:couv>=1?C.ambre:C.rouge,bold:couv<1},
          mp.fournisseur||"--",
        ];
      }),
      [100,65,65,55,65,90]
  ...
  pdfTableau(doc,['Action','Impact attendu','Priorite','Delai'],
    [
      ["Reduire les arrets non planifies","+5 a +8% capacite","Haute","1 a 3 mois"],
      ["Optimiser les changements de format","+3 a +5% capacite","Moyenne","1 mois"],
      ["Ajouter un 3eme shift","+50% capacite","Haute","3 a 6 mois"],
  ...
  pdfTableau(doc,['Indicateur','Valeur (FCFA)','Base de calcul'],
    [
  ...
  if(credits_total>0){
    ...
    pdfTableau(doc,['Parametre','Valeur','Interpretation'],
      [
        ...
        [{text:"Coefficient remboursement",bold:true},{text:coefRemb.toFixed(4),bold:true,color:coefRemb>=0.05?C.vert:C.rouge},coefRemb>=0.10?"Solide (>=10%/mois)":coefRemb>=0.05?"Acceptable (5-10%/mois)":"Faible (<5%/mois)"],
    ...
    if(dureeRemb>0){
      ...
      for(let i=1;i<=Math.min(dureeRemb,12);i++){
        ...
        const remb=Math.min(capaciteRemb,restant); restant-=remb;
        lignes.push([label,fmt(capaciteRemb)+" F",fmt(remb)+" F",fmt(Math.max(0,restant))+" F",
          restant<=0?{text:"Solde",bold:true,color:C.vert}:"En cours"]);
        if(restant<=0) break;
      }
